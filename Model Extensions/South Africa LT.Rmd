...
SubsetPeriod <- "1971-01-30::2016-12-31"
...
AllAssets.df  <- mutate(AllAssets.df,
  ...
  "GTAA.ret" = (JALSH.timingret + MSCIWORLD.timingret + SA10YR.timingret + GSCI.timingret)/4, # + JASPY.timingret)/5,
  "BUYANDHOLD.ret" = (JALSH.ret + MSCIWORLD.ret + SA10YR.ret + GSCI.ret)/4 # + JASPY.ret)/5
)
...
```{r}
SubsetPeriod <- "1972-01-31::2016-12-31"
#for calculating the sharpe ratio later
Rf.ret <- Return.calculate(TBILLYIELDS)[-1,]
Rf.ret <- Rf.ret[SubsetPeriod]

Returns <- cbind.xts(AllAssets.xts$GSCI.ret, AllAssets.xts$GSCI.timingret)[SubsetPeriod]
...
chart.TimeSeries(CumReturns,
  main = paste("GSCI Buy and Hold vs Timing ",substr(SubsetPeriod,1,4),"-",substr(SubsetPeriod,13,16),", Log Scale",sep = ""),
  date.format = "%Y",
  colorset = c(4,2),
  ylab = "",
  ylog = TRUE,
...
chart.Drawdown(Returns,
  main = paste("GSCI Buy and Hold vs Timing Drawdowns ",substr(SubsetPeriod,1,4),"-",substr(SubsetPeriod,13,16),", Non-log Scale",sep = ""),
  date.format = "%Y",
  colorset = c(4,2),
  ylab = "Drawdown",
