...
```{r}
library(dplyr)
getSymbols(c("SP500LT","TBillYields"),
           src = "csv",
           dir="../Data/CSV files",
           col.names=c("Close"))

AllAssets <- cbind.xts(Cl(INDU),
                       Cl(TBILLYIELDS))#,Cl(US10YR), Cl(GSCI),Cl(NAREIT))    
...
AllAssets.df  <- mutate(AllAssets.df, 
  "10mSMA" = rollmean(INDU.Close,10,fill = 0,align = "right"),
  "Asset.ret" = c(0,diff(INDU.Close)/lag(SP500.Close)[-1]),
  "Rf.ret" = lag((TBILLYIELDS.Close/100)/12),
  "signal" = lag(ifelse(INDU.Close>`10mSMA`,1,0)),
...
charts.PerformanceSummary(Returns,
                          main = "Dow Jones Returns vs Timing Total Returns (1973-2012)",
                          date.format = "%Y",
                          colorset = c(4,2),
                          ylog = FALSE,
                          minor.ticks =FALSE)
